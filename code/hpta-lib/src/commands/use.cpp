...
bool Use::interprete(Context &context, const std::vector<std::string> &token)
{
    ...
    auto item = std::find_if(std::begin(items_in_inventory), std::end(items_in_inventory), [&](auto val) {
        return Hpta_strings::equals_ignorecase(val.item.get_name(), item_name);
    });
